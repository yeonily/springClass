...
import com.example.mybasic.domain.entity.SuperCar;
...
//        4.엔티티의 Alias는 필수로 작성해야 한다.
        return entityManager.createQuery("select s from SuperCar s", SuperCar.class).getResultList();//jpql
        //getSingleResult 1개
        //getResultList 여러개
    }
...
//    페이징
    public List<SuperCar> findAllPaging(int offset, int limit){
        ...
        return entityManager.createQuery(q, SuperCar.class)
                ...
                .setMaxResults(limit) //1페이지에서 2개 들어온다.
                .getResultList();
    }

//    수정(벌크 연산), 영속성 컨텍스트를 무시하고 SQL 반영, 영속성 컨텍스트를 비워주고 실행해야 한다!!
    //하나 수정할 때는 setter쓰면 됨. 한번에 여러 개 수정할 때 JPQL로 수정가능.
    public void bulkUpdate(LocalDateTime superCarReleaseDate){
        ...
        entityManager.createQuery(q).setParameter("superCarReleaseDate", superCarReleaseDate).executeUpdate();
    }
}
