...
//        최신 게시글 5개에 2개씩 댓글 달기
        int amount = 5;
        int replyCount = 2;
        int totalCount = amount * replyCount;

        Criteria criteria = new Criteria();
        criteria.createCriteria(1, amount);
        List<Long> boardNumbers = new ArrayList<>();
        boardMapper.getList(criteria).stream().map(board -> board.getBoardNumber()).forEach(boardNumbers::add);

        for (int i = 0; i < totalCount; i++) {
            ReplyVO replyVO = new ReplyVO();
            replyVO.setBoardNumber(boardNumbers.get(i % amount));
            replyVO.setReplyContent("후..reply" + (i + 1));
            replyVO.setReplyWriter("나다");
...
//        });
    ...
    public void selectAllTest(){
        replyMapper.selectAll(542l).stream().map(ReplyVO::toString).forEach(log::info);
    }
    ...
    @Test
    public void selectCountOfReply(){
        int amount = 5, replyCount = 2, totalCount = amount * replyCount;
